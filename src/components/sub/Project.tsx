    ...
    return (
        <>
            <motion.div
                ...
                viewport={{ once: true }}
                transition={{}}
                className="relative w-full lg:w-[470px] h-max border border-yellow-400 rounded-lg cursor-pointer">
                ...
                <motion.div onClick={() => setShow(!show)}
                    ...
                    className="absolute top-0 bg-white/95 dark:bg-zinc-700 w-full h-full p-2 gap-y-2 flex flex-col justify-center items-center rounded-lg ">
                    <h2 className="text-md sm:text-lg font-bold tracking-wide text-gray-500 dark:text-gray-300">{data.name}</h2>
                    <p className="text-xs sm:text-sm text-justify text-gray-500 dark:text-gray-400 first-letter:pl-4">{data.desc}</p>
                </motion.div>
            </motion.div>
