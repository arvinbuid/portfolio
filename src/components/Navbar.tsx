            ...
            <div className="flex flex-col gap-y-2 sm:gap-y-3">
                {navbarData.map((item, index) => (
                    <a href={`#${item.id}`} key={index} className="group flex flex-col items-center gap-y-2 ">
                        ...
                        <span className={`text-[12px]  opacity-0 group-hover:opacity-100 group-hover:translate-x-0 duration-300 ease-in transition-all text-center tracking-wide ${index % 2 === 0 ? "translate-x-2" : "-translate-x-2"} ${item.id === id && "-translate-x-0 opacity-100"}`}>{item.name}</span>
                    </a>
                ))}
            </div>
            <p className="relative flex flex-col justify-center items-center text-sm text-gray-500 gap-y-2">
                <span className="absolute left-1/2 origin-bottom-left w-max flex items-center -rotate-90 ml-3">{copyRightIcon} {date}</span>
            </p>
        </div>
    )
}
