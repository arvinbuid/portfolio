...
function Navbar({ id }: NavbarProps) {
    ...
    return (
        <div className="w-[60px] md:w-[70px] fixed h-full top-0 left-0 flex flex-col justify-between border-r border-gray-200 px-4 py-10 z-100">
            <a href="#">
            ...
            <div className="flex flex-col gap-y-2 sm:gap-y-3">
                {navbarData.map((item, index) => (
                    <a href={`#${item.id}`} key={index} className="group flex flex-col items-center gap-y-2 ">
                        ...
                        <span className={`text-[10px] lg:text-[12px] dark:text-gray-300 opacity-0 group-hover:opacity-100 group-hover:translate-x-0 duration-300 ease-in transition-all text-center tracking-wide ${index % 2 === 0 ? "translate-x-2" : "-translate-x-2"} ${item.id === id && "-translate-x-0 opacity-100"}`}>{item.name}</span>
                    </a>
                ))}
            </div>
